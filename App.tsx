...
function serverBaseUrl() {
  const configured = process.env.EXPO_PUBLIC_APP_SERVER_URL?.trim();
  if (configured) {
  ...
  return Promise.all(
    payload.variants.map(async (variant, index) => {
      ...
      const recipe = withRecipeMeta(
        normalizeRecipe({
          ...variant.recipe,
          id: `source-variant-${variant.provider}-${Date.now()}-${index}`,
          source: 'generated',
          locale: language,
        }),
...
async function fetchRecommendations(language: Language): Promise<Recipe[]> {
  ...
  const recipes = Array.isArray(response?.recipes)
    ? response.recipes.map(normalizeRecipe).filter((recipe) => recipe.title)
    : [];
  if (!recipes.length) {
