...
if (-not $aiServer.Reachable) {
  Write-Host "Warning: local AI server health check did not respond. Login/profile features will not work until the server is reachable." -ForegroundColor Yellow
} elseif ($Mode -eq 'tunnel') {
  Write-Host "Note: Expo tunnel exposes the bundle, not your local AI server. Secret AI keys stay on the server; if the server is unreachable, the app uses safe local fallbacks." -ForegroundColor DarkYellow
}
