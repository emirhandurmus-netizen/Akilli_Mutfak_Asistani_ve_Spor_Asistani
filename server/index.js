...
async function findRecommendedRecipes(language) {
  const queries =
    normalizeLanguage(language) === 'tr'
      ? [
          'pratik yemek tarifi malzemeler kalori',
          'kolay aksam yemegi tarifi',
          'saglikli ev yemegi tarifi',
        ]
      : [
          'easy healthy dinner recipe ingredients calories',
          'quick home recipe ingredients',
          'balanced dinner recipe nutrition',
        ];
  const domains = recipeDomainsForLanguage(language);
  const found = new Map();
  for (const query of queries) {
    const urls = await searchWebPages(query, domains, 8).catch(() => []);
    const parsed = await Promise.allSettled(
      urls.slice(0, 5).map((item) => recipeFromUrl(item.url, query, language))
    );
    for (const result of parsed) {
      ...
      const recipe = {
        ...(await addRecipeVideo(result.value, result.value.title, language).catch(
          () => result.value
        )),
        source: 'recommended',
      };
      found.set(normalizeLookupText(recipe.title), recipe);
      if (found.size >= RECOMMENDATION_COUNT) {
...
app.post('/api/search/recipe', searchRateLimit, async (req, res) => {
  try {
    ...
    try {
      rememberSearchResponse({
        cacheKey: searchCacheKey('recipe', recipeCacheParameters(ingredientsText, language)),
        kind: 'recipe',
        payload: { recipe: fallback },
        source: 'fallback',
      });
