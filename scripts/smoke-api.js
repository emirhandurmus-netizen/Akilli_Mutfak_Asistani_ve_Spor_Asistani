...
const PORT = Number(process.env.SMOKE_APP_SERVER_PORT || 3101);
...
async function waitForHealth(timeoutMs = 30000) {
  ...
  while (Date.now() < deadline) {
    try {
      ...
      if (status === 200 && payload.ok && payload.service === 'app-server') {
        return payload;
      }
...
async function main() {
  ...
  try {
    ...
    const recipeResponse = await request('/api/search/recipe', {
      method: 'POST',
      body: {
    ...
    const translationResponse = await request('/api/search/translate-recipe', {
      method: 'POST',
      body: {
    ...
    const workoutResponse = await request('/api/search/workout-plan', {
      method: 'POST',
      body: {
    ...
    const appHelpResponse = await request('/api/search/app-help', {
      method: 'POST',
      body: {
    ...
    const cachedAppHelpResponse = await request('/api/search/app-help', {
      method: 'POST',
      body: {
        question: appHelpQuestion,
        language: 'en',
    ...
    const variantsResponse = await request('/api/search/recipe-variants', {
      method: 'POST',
      body: {
        ingredientsText: 'chicken, rice, tomato',
        language: 'en',
    ...
    assert(
      Array.isArray(variantsResponse.payload.variants) &&
        variantsResponse.payload.variants.length >= 1,
      'Recipe-variants endpoint did not return expected source variants.'
    );
    ...
    console.log(
      'Validated endpoints: health, app-config, admin config, recipe search, source variants, translate-recipe, workout-plan, app-help cache, auth, profile, user data.'
    );
  } finally {
