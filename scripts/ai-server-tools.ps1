...
function Wait-AiServerReady {
  ...
  do {
    $health = Get-AiServerHealth -Port $Port
    if ($health -and $health.ok -and $health.service -eq 'ai-server') {
      return $true
    }
...
function Ensure-LocalAiServer {
  ...
  $env:AI_SERVER_PORT = "$ApiPort"
  $env:EXPO_PUBLIC_AI_SERVER_URL = $apiUrl

  $health = Get-AiServerHealth -Port $ApiPort
  if (-not $Restart -and $health -and $health.ok -and $health.service -eq 'ai-server') {
  ...
  return [pscustomobject]@{
    ...
    LanIp = $lanIp
    Started = $true
    Reachable = (Wait-AiServerReady -Port $ApiPort)
  }
}
